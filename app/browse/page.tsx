...
function BrowseCatalog() {
  ...
  useEffect(() => {
    ...
    const fromBank = searchParams.get("bank");
    if (fromFilter && FILTERS.some((f) => f.id === fromFilter)) {
      setActiveFilters([fromFilter]);
    }
    if (fromBank) setBank(fromBank);
    setInitialized(true);
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-16">
      <div className="max-w-2xl mb-10">
        <h1 className="text-h1 text-ink mb-3">All credit cards</h1>
        <p className="text-body-lg text-inkMuted">
          Every card in our catalogue, filterable by what matters to you.
        </p>
      </div>
      ...
      {compareIds.length > 0 && (
        <div className="card-panel p-4 mb-8 flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <button onClick={() => setCompareIds([])} className="btn-ghost text-small">
              Clear
            </button>
            <button
      ...
      {loading ? (
        ...
        <div className="text-body text-inkMuted mb-20">
